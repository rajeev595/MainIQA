function getScores(distortion, data, PathToData, PathToResults)
    ...
    f = load('E:/MATLABe/GeoPath/DataForScores/W33/Source_PCA.mat');
    Ps = f.s_coeff;
    scores = [];
    
    for i = 1 : noImages
        f = sprintf('Patches_img%d.mat',i);     % Name of the Target Patches file
        f = strcat(PathToPatches,f);            % Adding path to the name    
        data = load(f);                         % Loading the data
        Xt = data.Xt;                           % Patches        
